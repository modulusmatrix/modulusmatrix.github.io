// MODULUS X - 1. Formül Açılımı Analitik Motoru (Kur'an & Hadis Entegrasyonlu)
...
function formulAnaliziniCalistir() {
    ...
    setTimeout(() => {
        ...
        if (zamanDelta > 0) {
            ...
            zamanCarpaniYonu = "+";
            zamanCarpaniAnalizi = "Anın vacibini yerine getirme dengesi hakim. Gecikme yok, doğrudan ihlasla eylem anı.";
        }

        // 2. YER DEĞİŞTİRME VE NİHAİ DEĞER HESABI
        const toplamBoyut = maddi + manevi + disBoyut;
        // İslami modelde manevi boyutun (ihlas, niyet) çarpanı her zaman en yüksektir
        ...
        if (manevi < 5) {
            referansMetni = `<strong>Kur'an Referansı:</strong> "İnsan için ancak çalıştığının karşılığı vardır." (Necm, 39)<br>
                            ...
                            <em>Analiz:</em> Manevi boyut kritik seviyede düşük. Niyet ekseni dünya menfaatine kaymış olabilir. Kalbi bir arınma şarttır.`;
            eylemselPlan = `<li><strong>1. Görev (Manevi Arınma):</strong> Eyleme başlamadan önce niyetinizi sadece Allah rızasına ve ilme endeksleyin. İstihare ve istişare kanallarını aktif edin.</li>
            ...
            referansMetni = `<strong>Kur'an Referansı:</strong> "Kararını verdiğin zaman artık Allah’a dayanıp güven (tevekkül et)." (Âl-i İmrân, 159)<br>
                            ...
                            <em>Analiz:</em> Manevi altyapı güçlü, ancak bu inancın salih amele ve maddi gayrete (cehde) dönüşmesi gerekiyor.`;
            eylemselPlan = `<li><strong>1. Görev (Süreklilik):</strong> Az da olsa devamlı olan ameller Allah katında sevimlidir. Projeniz için her gün aksatmadan en az 30 dakika eylemsel çalışma disiplini kurun.</li>
                            <li><strong>2. Görev (Maddi Sebeplere Sarılma):</strong> Deveyi bağlayıp sonra tevekkül etmek adına, lojistik eksiklerinizi (eğitim malzemesi, finansal bütçe) listeyip ilk somut adımı atın.</li>`;
        }

        if (maddi <= 3) {
            eylemselPlan += `<li><strong>3. Görev (İnfaq ve Sabır):</strong> Maddi imkanların kısıtlı olması bir engel değildir. Mevcut sıfır sermayeli dijital imkanları (GitHub, AI araçları) sonuna kadar zorlayarak 'fiili dua' edin.</li>`;
        } else {
            eylemselPlan += `<li><strong>3. Görev (Maddi Kaynak Yönetimi):</strong> Elinizdeki maddi gücü israf etmeden, helal dairede projenin büyümesi için infak ve yatırım dengesiyle kullanın.</li>`;
        }
        ...
        sonucPaneli.innerHTML = `
            <div style="margin-top: 25px; padding: 20px; border: 1px solid rgba(0,242,254,0.2); background: #172030; border-radius: 12px; text-align: left;">
                ...
                <div style="background: #0c111a; padding: 12px; border-radius: 8px; font-family: monospace; font-size: 0.9rem; color: #fff; margin-bottom: 20px; text-align: center; border: 1px solid rgba(0,242,254,0.1);">
                    Z(${zamanCarpaniYonu}) * ((${maddi}YD<sub>maddi</sub> ± ${manevi}YD<sub>manevi</sub>) * B<sub>[${birlesmeGirdi.substring(0,8)}]</sub>) = <span style="color: ${netDegerRenk}; font-weight: bold;">${netDegerSembolu} (${netSkor.toFixed(0)})</span>
                </div>
                ...
                <div style="font-size: 0.9rem; line-height: 1.6;">
                    <p style="color: #00ff87; display: ${isPositive ? 'block' : 'none'};"><strong>[+ DEĞER] Doğru Yer Değiştirme (Maksimum Kazanç):</strong> İlahi rızaya ve dünya esbabına uygun hareket ediliyor. Bu eylemin neticesinde hem ilmi/manevi gelişim hem de berekete dayalı maddi bir fetih öngörülüyor.</p>
                    <p style="color: #ff0055; display: ${!isPositive ? 'block' : 'none'};"><strong>[- DEĞER] Yanlış Yer Değiştirme (Maksimum Kayıp):</strong> Boyut dengeleri oturmamış. Acelecilik (veya erteleme) ve niyet zaafiyeti sebebiyle zaman israfı ve hayal kırıklığı yaşanma riski yüksektir.</p>
